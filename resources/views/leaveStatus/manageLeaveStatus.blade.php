                    ...
                    <div class=" rounded h-100 p-4">
                        <div class="row">
                            <div class="col-md-6 mt-4 ">
                                <h3 class=" text-dark text-start">Manage Leave Status</h3>
                            </div>
                                        ...
                                        <tr>
                                            ...
                                            <td scope="row" class="d-flex">
                                                @if ($leave->trashed())
                                                    ...
                                                    <a href="{{ route('leave-status.restore', $leave->id) }}"
                                                        data-bs-toggle="tooltip" title="Restore Leave Status"
                                                        class="text-success btn">
                                                        <i class="fas fa-trash-restore"></i>
                                                    </a>
                                                @else
                                                    <!-- Edit and Delete icons when the record is not trashed -->
                                                    <a href="{{ route('leave-status.edit', $leave->id) }}"
                                                        data-bs-toggle="tooltip" title="Edit Leave Status"
                                                        class="text-primary btn"><i class="fa fa-edit"></i></a>
                                                    <form action="{{ route('leave-status.destroy', $leave->id) }}" method="post">
                                                        @csrf
                                                        @method('DELETE')
